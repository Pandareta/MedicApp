...
const controllerAuth = {
        ...
        try {
          ...
          if (patient) {
            const tokenPatient = Jwt.sign(
              { user_id: patient.id },
              process.env.TOKEN_SECRET_PATIENT || "pacientetoken"
            );
            const data = {
              name: patient.name,
              email: patient.email,
          ...
          if (doctor) {
            const tokenDoctor = Jwt.sign({ user_id: doctor.id }, process.env.TOKEN_SECRET_DOCTOR || "doctortoken");
            const data = {
              name: doctor.name,
              email: doctor.email,
              rol: "DOCTOR",
          ...
          if (admin) {
            const tokenAdmin = Jwt.sign({ user_id: admin.id }, process.env.TOKEN_SECRET_ADMIN || "admintoken");
            const data = {
              name: admin.name,
              email: admin.email,
              rol: "ADMIN",
  ...
  logGoogle: async (req, res, next) => {
    try {
      ...
      if (patient) {
        const tokenPatient = Jwt.sign({ user_id: patient.id }, process.env.TOKEN_SECRET_PATIENT || "pacientetoken");
        const data = {
  ...
  forgotPassword: async (req, res, next) => {
    ...
    try {
      ...
      let token;
      if (patient) {
        token = Jwt.sign({ user_id: patient.id }, process.env.TOKEN_SECRET_PATIENT || "pacientetoken");
      }
      if (doctor) {
        token = Jwt.sign({ user_id: doctor.id }, process.env.TOKEN_SECRET_DOCTOR || "doctortoken");
      }
